...
router.post("/add-discussion", grantAccess(), async (req, res) => {
  try {
    ...
    const surgery = await Surgery.findById(surgeryId);
    const isAllowed = await checkPermission(surgery, user);
    if (isAllowed) {
...
router.post("/check-scenario-answer", async (req, res) => {
  ...
  textData =
    ...
    "; Answer - " +
    answer;
  const ask = `; Given above is the description of a surgery, a scenario along with a question and a surgeons answer to that scenario question. 
